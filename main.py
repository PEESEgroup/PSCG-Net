...
from data import CIFData2, collate_pool, get_train_val_test_loader
from model.cgcnn import CrystalGraphConvNet
from model.pscgnet import PSCGNet

parser = argparse.ArgumentParser(description='PSCGNet')
...
parser.add_argument('--model', choices=['cgcnn', 'pscgnet'],
                    default='pscgnet', help='training model')
...
data_dir = os.path.join('runs', 'datasets', args.task, args.task_name)
...
def main():
    ...
    # load data
    dataset = CIFData2(*args.data_options, max_num_nbr=args.max_num_nbr, radius=args.radius, output_dir=data_dir, task_name=args.task_name, task=args.task)
    ...
    if 'cgcnn' in args.model:
        ...
        model = PSCGNet(orig_atom_fea_len, nbr_fea_len,
                                atom_fea_len=args.atom_fea_len,
                                n_conv=args.n_conv,
                                h_fea_len=args.h_fea_len,
                                n_h=args.n_h,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        save_checkpoint({
            ...
            'args': vars(args)
        }, is_best, prefix=output_dir)

        save_history(output_dir)

    # test best model
    print('---------Evaluate Model on Test Set---------------')
    # best_checkpoint = torch.load(os.path.join(output_dir, 'model_best.pth.tar'))
...
def train(train_loader, model, criterion, optimizer, epoch, normalizer):
    ...
    if args.task == 'regression':
        history['train_loss'].append(float(losses.avg.data))
        history['train_mae'].append(float(mae_errors.avg.data))
    else:
        history['train_loss'].append(float(losses.avg))
        history['train_auc'].append(float(auc_scores.avg))
...
def validate(val_loader, model, criterion, normalizer, test=False):
    ...
    if args.task == 'regression':
        print(' {star} MAE {mae_errors.avg:.3f}'.format(star=star_label,
                                                        mae_errors=mae_errors))
        history['val_loss'].append(float(losses.avg))
        history['val_mae'].append(float(mae_errors.avg.data))
        with open(os.path.join(output_dir, 'test.txt'), "w", encoding="utf-8") as file:
            file.write(' {star} MAE {mae_errors.avg:.3f}'.format(star=star_label, mae_errors=mae_errors))
        return mae_errors.avg
    else:
        print(' {star} AUC {auc.avg:.3f}'.format(star=star_label,
                                                 auc=auc_scores))
        history['val_loss'].append(float(losses.avg))
        history['val_auc'].append(float(auc_scores.avg))
        with open(os.path.join(output_dir, 'test.txt'), "w", encoding="utf-8") as file:
            file.write(' {star} AUC {auc.avg:.3f}'.format(star=star_label, auc=auc_scores))
        return accuracies.avg

def save_history(output_dir):
    # Save to JSON
...
def save_checkpoint(state, is_best, prefix, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filename, os.path.join(prefix, 'model_best.pth.tar'))
